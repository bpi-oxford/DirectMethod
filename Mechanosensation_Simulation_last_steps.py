...
#os.chdir(r'\\kirpc541.kennedy.ox.ac.uk\sambashare2\Marcel\Mechanosensation_Simulation')
os.chdir(r'/media/Data3/Marcel/Mechanosensation_Simulation')
import shutil
...
def generate_image_confocal(beads, n_im, pxl_size):
    bg  = 6
    fwhm = .2
    ...
    # _image = (_image / np.max(_image)) # normalize image between 1 and 0
    # _image = np.around((max_intensity-bg) * _image)
    # set pixel background 
    _image = _image + bg
    
    #_image = _image + np.random.normal(loc=0, scale=0, size=np.shape(_image))
    _image = np.random.poisson(lam = _image)   # add shot noise

    return np.around(_image)

def generate_image_tirf_sim(beads, n_im, pxl_size):
    bg = 2100, 
    max_intensity = 8000
    fwhm = 0.1
    # create empty synthetic image:
    ...
    # Post enditing / adapt to microscope 

    # = _image * max_intensity
    # set pixel background 
    _image_bg = _image + bg
    
    _image_noise = _image_bg + np.random.normal(loc=0, scale=250, size=np.shape(_image_bg))
    _image_shot = np.random.poisson(lam = _image_noise)   # add shot noise
    _image_final = np.array(np.around(_image_shot))
    return _image_final

def synthetic_image_pairs(__beads, _dx, _dy, pxl_size, scope):

    # scale normed beads 
    _x_bead =  __beads[:,0] * n_points_xy
    _y_bead =  __beads[:,1] * n_points_xy
    ...
    _ux_beads = griddata((__xx.ravel(),__yy.ravel()), _dx.ravel(), (_x_bead,_y_bead), method = 'cubic')
    _uy_beads = griddata((__xx.ravel(),__yy.ravel()), _dy.ravel(), (_x_bead,_y_bead), method = 'cubic')

    # Generate synthetic Image and use trackpy to retrace Traction field.
    # plt.scatter(_ux_beads,_uy_beads)
    # plt.show()
    ...
    if scope == 'tirf-sim':
        _image_1 = generate_image_tirf_sim(__beads, n_points_xy, pxl_size) 
        _image_2 = generate_image_tirf_sim(_uu_beads, n_points_xy, pxl_size)

    if scope == 'confocal':
        _image_1 = generate_image_confocal(__beads, n_points_xy, pxl_size) 
        _image_2 = generate_image_confocal(_uu_beads, n_points_xy, pxl_size)


    return np.array([_image_1, _image_2])
#%%
...
def pollute_displacement(name):
    ...
    # Save Beads
    #np.savez(save_beads, beads)

    #Generate Synthetic Images and save them
    images = synthetic_image_pairs(beads, dx, dy, pixel_size, scope)
    images = images.astype(np.int16)
    file_name= os.getcwd() + '/' + tmp_image_loc + '/synthetic_' + name[:-4] + '.tif'
    tifffile.imwrite(file_name, images)
...
def generate_ux_uv(name):
    ...
    d = r
    d_max = 0.25/pixel_size # Fix max linking distance at 250 nm.
    
    if d % 2 == 0: 
        d = d+ 1 

    test_set =  tp.batch(test_image, d, separation = sep , minmass = mass) 
    ...
    _uux = np.nan_to_num(_uux, nan = 0)*pixel_size*1000
    _uuy = np.nan_to_num(_uuy, nan = 0)*pixel_size*1000
...
def svd_metric(im1,im2): 
    ...
    metric = np.sqrt(np.sum((s1-s2)**2))/np.sum(s1 +s2)
    return metric
#%%
def accuracy_image(_gt,_im): 
    def _count_objects(img):

        # Binary image, post-process the binary mask and compute labels
        mask = img > .8 * np.max(img)
        ...
        for index in range(1, labels.max()):
            ...
            for prop_name in properties:
                area = getattr(props[index], prop_name)
                if area > 250: 
                    A.append(area)
                    # print(area)
        # plt.imshow(mask)
...
def precision_image(_im,_gt):
    ...
    _binary_image = _im > 0.8 * np.max(_im)
    _binary_gt = _gt > 0.8 * np.max(_gt)
    
    if  np.mean(_gt[_binary_gt])*.8 <= np.mean(_im[_binary_image]) < np.mean(_gt[_binary_gt]) * 1.2:
        _precision = 1
...
if __name__ == "__main__":
    ...
    # adjust variables
    if scope == 'confocal':
        microscope_resolution = 0.2
        r = int(5 * (microscope_resolution/pixel_size))
        sep = r/5
        if r % 2 == 0: 
            r = r+ 1
    ...
    #force_list = np.linspace(0.001, 0.01,20)
    #young_list = [0.4, 1,10]
    # force_list = [1,5]
    ...
    scale = 0.03 # 30 nm per  pxl for TIRF-SIM
    ...
    for k in [1,2,3,4,5,6,7,8]: 
        ...
        for i in tqdm(range(1,n+1)):
            ...
            for f in force_list:
                for e in young_list:
                    ...
                    path_filtered = tmp_filter_loc + '/filtered_synthetic_displacement_' + str(f) + '_nN_on_' + str(e) +'_kPa.npz'
                ...
                peak_tractions.append(tmp_peak_tractions)
        ...
        mean_traction_list_raw = np.reshape(means(peak_tractions),s)
        vmax = None
        ...
        plt.figure(dpi = 150)
        plt.imshow(mean_traction_list_raw, origin = 'lower', vmax = None)
        ...
        plt.xlabel('E [kPa]')
        plt.ylabel('F [nN]')
        plt.title('raw displacement for every peak.')
        plt.colorbar()
        plt.show()    
        ...
        save_path = 'results_' + scope + 'run_'+ str(k) +'.npz'
        np.savez(save_path, mean_svd, mean_accuracy, mean_precision)
#%%
